...
# Кэш работающего Nitter инстанса
...
# Nitter instances - более надёжные для Twitter RSS
NITTER_INSTANCES = [
...
class NewsAnalyzer:
    ...
    async def fetch_twitter_posts(self, accounts: List[str] = None) -> List[NewsEvent]:
        """
        Получить твиты от ключевых аккаунтов через:
        1. Nitter (самый надёжный)
        2. RSSHub
        3. RSS.app (fallback)
        """
        events = []
        ...
        # 1. Пробуем Nitter (самый надёжный для Twitter RSS)
        nitter_events = await self._fetch_twitter_via_nitter(accounts_to_fetch)
        if nitter_events:
            logger.info(f"[TWITTER] ✅ Got {len(nitter_events)} tweets via Nitter")
            return nitter_events
        
        # 2. Fallback на RSSHub
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not working_instance:
                logger.warning("[TWITTER] ❌ No working RSSHub instance found, trying RSS.app...")
                # 3. Последний fallback - RSS.app
                try:
                    ...
                    logger.warning(f"[TWITTER] RSS.app also failed: {e}")
                return events
    ...
    async def _fetch_twitter_via_nitter(self, accounts: List[str]) -> List[NewsEvent]:
        """Получить твиты через Nitter RSS - самый надёжный метод"""
        global _working_nitter_instance, _nitter_last_check
        events = []
        working_instance = None
        now = datetime.now()
        
        # Используем кэшированный инстанс если проверяли недавно (5 минут)
        ...
        async with aiohttp.ClientSession() as session:
            # Ищем работающий Nitter инстанс
            if not working_instance:
                for instance in NITTER_INSTANCES:
                    try:
                        # Тестируем на популярном аккаунте
                        ...
                        async with session.get(
                            ...
                            if resp.status == 200:
                                ...
                                if '<item>' in content and '<title>' in content:
                                    ...
                                    logger.info(f"[NITTER] ✅ Found working instance: {instance}")
                                    break
                                else:
                                    logger.debug(f"[NITTER] {instance} returned invalid RSS")
                            else:
            ...
            if not working_instance:
                logger.warning("[NITTER] ❌ No working Nitter instance found")
                return events
            ...
            for username in accounts:
                try:
                    ...
                    async with session.get(
                        ...
                        if resp.status != 200:
                            logger.debug(f"[NITTER] @{username} returned {resp.status}")
                            continue
                    ...
                    if not items:
                        logger.debug(f"[NITTER] @{username} no items found")
                        continue
                    ...
                    for item in items[:5]:  # Последние 5 твитов
                        ...
                        # Пропускаем RT: и слишком короткие
                        if not title or len(title) < 15 or title.startswith('RT:') or title.startswith('R to @'):
                            continue
                        ...
                        # Всегда релевантно если от важных источников
                        if account_info.get('type') in ['regulator', 'central_bank', 'government', 'politician', 'exchange']:
                            is_relevant = True
                        # Или если содержит крипто-ключевые слова
                        elif any(kw in title_lower for kw in ['btc', 'bitcoin', 'eth', 'ethereum', 'crypto', 'sol', 'solana', 
                                                              'pump', 'dump', 'long', 'short', 'bullish', 'bearish',
                                                              'sec', 'etf', 'fed', 'rate', 'inflation', 'tariff']):
                            is_relevant = True
                        # Или по ключевым словам аккаунта
                        elif any(kw.lower() in title_lower for kw in account_keywords):
                            is_relevant = True
                        ...
                        news_hash = self._get_news_hash(title, f"nitter_{username}")
                        if news_hash in self.seen_news:
                            continue
                        ...
                        link_match = re.search(r'<link[^>]*>([^<]+)</link>', item)
                        url = link_match.group(1).strip() if link_match else f"https://twitter.com/{username}"
                        # Конвертируем Nitter URL в Twitter URL
                        if working_instance in url:
                            url = url.replace(working_instance, 'https://twitter.com')
                        ...
                        # Impact на основе типа аккаунта
                        impact_str = account_info.get('impact', 'MEDIUM')
                        try:
                            base_impact = NewsImpact[impact_str]
                        except KeyError:
                        ...
                        events.append(event)
                        logger.debug(f"[NITTER] ✅ @{username}: {title[:50]}...")
                    
                except Exception as e:
                    logger.debug(f"[NITTER] Error fetching @{username}: {e}")
                    continue
        ...
        if events:
            logger.info(f"[NITTER] ✅ Fetched {len(events)} tweets from {len(accounts)} accounts")
