...
class NewsAnalyzer:
    ...
    async def fetch_twitter_posts(self, accounts: List[str] = None) -> List[NewsEvent]:
        """
        Получить твиты от ключевых аккаунтов через RSS.app (YULA bundle).
        """
        accounts_to_fetch = accounts or PRIORITY_TWITTER_ACCOUNTS[:10]
        logger.info(f"[TWITTER] Starting fetch for {len(accounts_to_fetch)} accounts via RSS.app...")
        try:
            events = await self._fetch_twitter_via_rss_app(accounts_to_fetch)
            if events:
                logger.info(f"[TWITTER] ✅ Got {len(events)} tweets via RSS.app")
            return events
        except Exception as e:
