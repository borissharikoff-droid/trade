...
4. Какое новое правило можно извлечь?"""
    ...
    async def should_take_trade(self, signal: Dict, market_data: Dict = None) -> Dict:
        ...
        try:
            ...
            system_prompt = """Ты - AI торговый советник. Цель: МАКСИМАЛЬНЫЙ ВИНРЕЙТ. Лучше пропустить сделку, чем взять плохую.

КРИТЕРИИ (оцени каждый и выведи итог):
1. Технический анализ (40%): качество сетапа (A+/A = хорошо, B = средне, C = плохо), R/R (мин 1:1.5), структура, объём
2. Историческая успешность (30%): WR по символу — если WR < 40% по символу, REJECT. Если были свежие убытки в таком же режиме — REJECT.
3. Рынок и новости (20%): BOB index (1-10: Bearish/Neutral/Bullish), корреляция с BTC, funding, OI. Если BOB противоречит направлению — REJECT.
4. Риск (10%): волатильность, зона входа, динамический SL/TP на основе ATR

СТРОГИЕ ПРАВИЛА:
- should_trade = false если: WR по символу < 40% (на >5 сделок), R/R < 1.2, BOB сильно противоречит, качество C/D без MTF alignment
- position_size_multiplier < 0.7 если confidence < 0.6
- Для каждого решения записывай key_concerns и key_strengths
- ОТКЛОНЯЙ сомнительные сделки: лучше 3 хороших чем 10 плохих
